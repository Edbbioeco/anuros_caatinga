...
lista_excluir <- matriz_final %>%
  dplyr::select(1, 6:159) %>%
  tidyr::pivot_longer(cols = 2:155,
                      values_to = "presence",
                      names_to = "species") %>%
  dplyr::group_by(species) %>%
  dplyr::summarise(abundance = presence %>% sum()) %>%
  dplyr::ungroup() %>%
